...
- We will use the [PhD thesis](https://repository.cern/records/r1fdr-2hh33) of P. Bélanger and its [`pytori`](https://github.com/pbelange/pytori/tree/main) package.
...
The first step is to represent $\rho$ using a grid of weighted macroparticle. In 2D, the grid has to be chosen on the tori $J_x$ at regular $\theta_x$ intervals.  Each macro-particle has to correspond to the value in $\rho(J_x, \theta_x)$  weighted by the area of the corresponding mesh. We assume a non resonant KAM region around a single fixed point of multiplicity 1 (the closed orbit). In other words the mesh is homothetic to a polar grid (via a normal form) assuming same $J_x$ at $\theta_x$ intervals.
